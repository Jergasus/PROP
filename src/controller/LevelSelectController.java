...
public class LevelSelectController {
    ...
    public LevelSelectController(DomainController domain, LevelCatalog catalog, User currentUser) {
        this.domain = domain;
    ...
    public void showLevelSelect() {
        while (true) {
            ...
            for (Difficulty d : Difficulty.values()) {
                ...
                domain.printMessage("\n--- " + d.getDisplayName() + " ---");
                domain.printLevelList(levels, currentUser.getAllLevelProgress());
            }

            domain.printMessage("\n  0. Volver al menu principal");
            String input = domain.askString("Selecciona un nivel (numero) o 0 para volver:");

            int choice;
            try {
                choice = Integer.parseInt(input.trim());
            } catch (NumberFormatException e) {
                domain.printMessage("Entrada no valida.");
                continue;
            }
            ...
            if (choice < 1 || choice > allLevels.size()) {
                domain.printMessage("Nivel no valido.");
                continue;
            }
    ...
    private void playLevel(Level level) {
        ...
        GameController gc = new GameController(game, domain, level.getLevelId(), level.getDifficulty());
        gc.play();
    ...
    private void handleLevelCompletion(String levelId, Difficulty difficulty, long timeMillis) {
        ...
        domain.printLevelResult(stars, pointsGained, timeMillis);

        // Persist user progress
        try {
            userRepo.saveUser(currentUser);
        } catch (IOException e) {
            domain.printMessage("Error al guardar progreso: " + e.getMessage());
        }
    }
}
